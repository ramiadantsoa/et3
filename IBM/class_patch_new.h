    ...
    public:
        Patch(double _x, double _y, double _q, int _M) {
            ...
            for(int i = 0; i<_M; i++) {
				...
				//this->fitness[i]= i ==0 ? 1 : exp(0.125*cos(2.0*PI*(_q - (double)i /(double)(_M-1)))- log(boost::math::cyl_bessel_i(0,1/0.125)));
                this->fitness[i]= i ==0 ? 1 : exp(0.125*cos(2.0*PI*(_q - (double)i /(double)(_M-1)))- 6.2);
            }
        }
